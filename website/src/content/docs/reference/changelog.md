...
- **Breaking (types):** SQLite `REAL` columns now resolve to `float64` instead of `float32`. SQLite's REAL storage class is defined as an 8-byte IEEE float — it has no 4-byte float — but the type mapping applied PostgreSQL's widths to every engine, where `real` genuinely is `float4`. Statically typed SQLite targets change accordingly (`f32` → `f64`, `float`/`getFloat` → `double`/`getDouble`); PostgreSQL is unaffected. SQLite's `INTEGER` has the same problem and is tracked separately in [#70](https://github.com/Goldziher/scythe/issues/70), since fixing it would retype every SQLite primary key
