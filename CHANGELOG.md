...
- New `migration` rule category and five canonical migration-safety rules under the `SC-MIG*` prefix: SC-MIG01 ban-drop-table, SC-MIG02 ban-drop-column, SC-MIG03 require-concurrent-index-creation, SC-MIG04 renaming-column, SC-MIG05 constraint-missing-not-valid. Each rule targets a class of irreversible or lock-prone Postgres DDL change that breaks zero-downtime deployments. All five declare `dialects = ["postgres"]`. Four new matcher functions back them: `drop_statement` (parameterised by `kinds = ["table", "column"]` so a single matcher serves both SC-MIG01 and SC-MIG02), `create_index_concurrency`, `alter_table_rename_column`, `constraint_missing_not_valid`. The matcher framework is unchanged.
