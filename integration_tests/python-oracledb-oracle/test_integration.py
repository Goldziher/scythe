"""Integration tests for scythe-generated oracledb queries."""
...
def get_database_url() -> str:
    """Read ORACLE_URL from environment."""
    url = os.environ.get("ORACLE_URL")
    if not url:
        print("ERROR: ORACLE_URL environment variable is not set", file=sys.stderr)
        sys.exit(1)
    return url
...
async def test_get_orders_by_user(conn, user_id: int, order_id: int) -> None:
