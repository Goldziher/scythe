...
{
    $url = getenv('MARIADB_URL');
    if ($url === false || $url === '') {
        fwrite(STDERR, "ERROR: MARIADB_URL environment variable is not set\n");
        exit(1);
    }
    return $url;
}
...
function parse_database_url(string $url): array
{
    $parts = parse_url($url);
    if ($parts === false) {
        fwrite(STDERR, "ERROR: Invalid MARIADB_URL format\n");
        exit(1);
    }
...
function test_get_user_by_id(PDO $pdo, string $user_id): void
{
...
function test_create_order(PDO $pdo, string $user_id): int
{
...
function test_get_orders_by_user(PDO $pdo, string $user_id, int $order_id): void
{
...
function test_delete_user(PDO $pdo, string $user_id): void
{
