...
# scythe:provenance v=0.15.0 backend=ruby-mysql2 engine=mysql schema=sch1:4332a9c33cb39297 queries=q1:f928696deb211f90 options=opt1:cbf29ce484222325
...
module Queries
  ...
  def self.create_order(client, user_id, total, notes)
    stmt = client.prepare("INSERT INTO orders (user_id, total, notes) VALUES (?, ?, ?)")
  ...
  def self.create_user(client, name, email, status)
    stmt = client.prepare("INSERT INTO users (name, email, status) VALUES (?, ?, ?)")
  ...
  def self.delete_user(client, id)
    stmt = client.prepare("DELETE FROM users WHERE id = ?")
    stmt.execute(id)
    nil
  end
