...
fun testCreateUser(conn: java.sql.Connection) {
    val name = "CreateUser"
    try {
        val user = conn.createUser("Alice", "alice@example.com", UserStatus.ACTIVE)
        if (user == null) {
...
fun testUpdateUserEmail(conn: java.sql.Connection) {
    ...
    try {
        conn.updateUserEmail("alice-updated@example.com", createdUserId)
        val user = conn.getUserById(createdUserId)
        if (user == null) {
...
fun testCreateOrder(conn: java.sql.Connection) {
    ...
    try {
        val order = conn.createOrder(createdUserId, BigDecimal("99.99"), "Test order")
        if (order == null) {
            fail(name, "returned null")
            return
        }
...
fun testGetOrdersByUser(conn: java.sql.Connection) {
    ...
    try {
        val orders = conn.getOrdersByUser(createdUserId)
        if (orders.size != 1) {
...
fun testGetOrderTotal(conn: java.sql.Connection) {
    ...
    try {
        val result = conn.getOrderTotal(createdUserId)
...
fun testListActiveUsers(conn: java.sql.Connection) {
    ...
    try {
        val users = conn.listActiveUsers(UserStatus.ACTIVE)
        if (users.isEmpty()) {
...
fun testGetUserOrders(conn: java.sql.Connection) {
    ...
    try {
        val results = conn.getUserOrders(UserStatus.ACTIVE)
        if (results.isEmpty()) {
...
fun testCountUsersByStatus(conn: java.sql.Connection) {
    ...
    try {
        val result = conn.countUsersByStatus(UserStatus.ACTIVE)
        if (result == null) {
            fail(name, "returned null")
            return
        }
...
fun testSearchUsers(conn: java.sql.Connection) {
    ...
    try {
        val users = conn.searchUsers("%Alice%")
        if (users.isEmpty()) {
...
fun testDeleteOrdersByUser(conn: java.sql.Connection) {
    ...
    try {
        val count = conn.deleteOrdersByUser(createdUserId)
        if (count != 1) {
