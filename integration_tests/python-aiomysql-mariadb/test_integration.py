...
def get_database_url() -> str:
    """Read MARIADB_URL from environment."""
    url = os.environ.get("MARIADB_URL")
    if not url:
        print("ERROR: MARIADB_URL environment variable is not set", file=sys.stderr)
        sys.exit(1)
    return url
