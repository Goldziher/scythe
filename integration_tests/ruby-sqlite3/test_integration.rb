...
def test_create_user(db)
  create_user(db, "Alice", "alice@example.com", "active")
...
def test_list_active_users(db)
  users = list_active_users(db, "active")
...
def test_update_user_email(db, user_id)
  update_user_email(db, "alice-new@example.com", user_id)
  user = get_user_by_id(db, user_id)
...
def test_create_order(db, user_id)
  create_order(db, user_id, "49.99", "Test order")
...
def test_delete_user(db, user_id)
  deleted_count = delete_orders_by_user(db, user_id)
