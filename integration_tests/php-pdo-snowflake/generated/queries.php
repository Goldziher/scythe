...
readonly class GetOrderTotalRow
{
    ...
    public static function fromRow(array $row): self
    {
        return new self(total_sum: $row['total_sum'] !== null ? (string) $row['total_sum'] : null);
    }
}
...
final class Queries
{
    ...
    {
        $stmt = $pdo->prepare(
            'SELECT id, name, email, active, metadata, created_at, updated_at FROM users WHERE id = :p1',
        );
        $stmt->execute(['p1' => $id]);
        $row = $stmt->fetch(\PDO::FETCH_ASSOC);
