...
def get_database_url
  ...
  if url.nil? || url.empty?
    warn "ERROR: MARIADB_URL environment variable is not set"
    exit 1
  end
  url
end
...
def test_create_user(conn)
  user = Queries.create_user(conn, "Alice", "alice@example.com", Queries::UsersStatus::ACTIVE)
...
def test_list_active_users(conn)
  users = Queries.list_active_users(conn, Queries::UsersStatus::ACTIVE)
