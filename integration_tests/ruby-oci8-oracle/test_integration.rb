...
def get_database_url
  ...
  if url.nil? || url.empty?
    warn "ERROR: ORACLE_URL environment variable is not set"
    exit 1
  end
  url
end
...
def test_create_order(conn, user_id)
  order = Queries.create_order(conn, user_id, 9999, "Test order")
...
def test_get_order_total(conn, user_id)
  ...
  assert_true(result.total_sum.to_i == 9999, "get_order_total total_sum")
...
def test_count_users_by_status(conn)
  ...
  assert_equal(1, result.active, "count_users_by_status active")
...
begin
  ...
  service = "#{uri.host}:#{uri.port || 1521}/#{uri.path&.delete_prefix('/')}"
  conn = OCI8.new(uri.user, uri.password, service)
