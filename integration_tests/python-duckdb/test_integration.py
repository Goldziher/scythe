"""Integration tests for scythe-generated duckdb queries."""
...
SCHEMA_PATH = Path(__file__).parent.parent / "sql" / "duckdb" / "schema.sql"


def get_database_url() -> str:
    """Read DUCKDB_PATH from environment."""
    url = os.environ.get("DUCKDB_PATH")
    if not url:
        print("ERROR: DUCKDB_PATH environment variable is not set", file=sys.stderr)
        sys.exit(1)
    return url
...
def setup_schema(conn: duckdb.DuckDBPyConnection) -> None:
    """Create schema from SQL file (fresh in-memory database each run, so
    there is nothing to drop -- unlike the on-disk engines, the CREATE
    SEQUENCE statements in the schema would fail on a second run against the
    same file)."""
...
def test_create_user(conn: duckdb.DuckDBPyConnection) -> int:
    """Test CreateUser query. Returns created user ID."""
    create_user(conn, name="Alice", email="alice@example.com", status="active")
    # Fresh in-memory database, sequence-backed id: the first row is 1.
    user = get_user_by_id(conn, id=1)
...
def test_get_user_by_id(conn: duckdb.DuckDBPyConnection, user_id: int) -> None:
...
def test_list_active_users(conn: duckdb.DuckDBPyConnection) -> None:
    """Test ListActiveUsers query."""
    users = list_active_users(conn, status="active")
...
def test_create_order(conn: duckdb.DuckDBPyConnection, user_id: int) -> int:
...
def test_delete_user(conn: duckdb.DuckDBPyConnection, user_id: int) -> None:
