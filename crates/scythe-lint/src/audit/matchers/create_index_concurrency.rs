...
pub fn match_create_index_concurrency(ctx: &LintContext<'_>, _args: &toml::Table) -> Vec<MatcherHit> {
    ...
    hit.bindings.insert("table".to_string(), idx.table_name.to_string());
    vec![hit]
}
...
mod tests {
    ...
    fn unnamed_index_uses_placeholder_binding() {
        ...
        let hits = match_create_index_concurrency(&ctx, &toml::Table::new());
        assert_eq!(hits.len(), 1);
        assert_eq!(hits[0].bindings.get("index").map(|s| s.as_str()), Some("<unnamed>"));
    }
}
