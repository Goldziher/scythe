...
impl CodegenBackend for PythonPyodbcBackend {
    ...
    fn file_header(&self) -> String {
        ...
        if self.row_type.is_stdlib_import() {
            ...
            let third_party = self.row_type.sorted_third_party_imports("import pyodbc  # noqa: F401");
            format!(
                "import datetime  # noqa: F401\n\
                 import decimal  # noqa: F401\n\
                 from enum import Enum  # noqa: F401\n\
    ...
    fn generate_grouped_query_fn(&self, request: &GroupedQueryFn<'_>) -> Result<String, ScytheError> {
        ...
        if sig.len() <= 88 {
            ...
            let _ = writeln!(out, "    conn: pyodbc.Connection,");
            if !params.is_empty() {
                let _ = writeln!(out, "    *,");
                for p in params {
        ...
        let _ = writeln!(out, "    cursor = conn.cursor()");
        let _ = writeln!(out, "    cursor.execute(");
        let _ = writeln!(out, "        \"\"\"");
...
mod tests {
    ...
    #[test]
    fn test_grouped_python_pyodbc_structs() {
        let backend = PythonPyodbcBackend::new("mssql").unwrap();
    ...
    #[test]
    fn test_grouped_python_pyodbc_query_fn() {
        let backend = PythonPyodbcBackend::new("mssql").unwrap();
        let query = make_grouped_query();
        let result = crate::generate_with_backend(&query, &backend).unwrap();
        ...
        assert!(
            query_fn.contains("pyodbc.Connection"),
            "missing conn type; got:\n{query_fn}"
        );
        assert!(
            query_fn.contains("cursor.execute("),
            "fn must use cursor.execute; got:\n{query_fn}"
        );
        assert!(
        ...
        assert!(
            ...
            "pyodbc fn must be synchronous; got:\n{query_fn}"
        );
    }
}
