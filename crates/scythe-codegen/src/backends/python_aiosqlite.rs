...
impl CodegenBackend for PythonAiosqliteBackend {
    ...
    fn file_header(&self) -> String {
        ...
        let header = if self.row_type.is_stdlib_import() {
            ...
            let third_party = self
                .row_type
                .sorted_third_party_imports("import decimal\nimport aiosqlite  # noqa: F401");
            format!(
                "from enum import Enum  # noqa: F401\n\
                 \n\
                 {third_party}\n\
    ...
    fn generate_query_fn(
        ...
        match &analyzed.command {
            QueryCommand::One | QueryCommand::Opt => {
                ...
                if params.is_empty() {
                    let _ = writeln!(out, "    async with conn.execute(\"\"\"{}\"\"\") as cursor:", sql);
                } else {
                    let _ = writeln!(
                        out,
            ...
            QueryCommand::Batch => {
                ...
                if params.is_empty() {
                    ...
                    let _ = writeln!(out, "    await conn.executemany(\"\"\"{}\"\"\", items)", sql);
                }
            ...
            QueryCommand::Exec => {
                ...
                if params.is_empty() {
                    ...
                    let _ = writeln!(out, "    await conn.execute(\"\"\"{}\"\"\", {})", sql, args_list);
                }
            }
    ...
    fn generate_grouped_query_fn(&self, request: &GroupedQueryFn<'_>) -> Result<String, ScytheError> {
        ...
        if sig.len() <= 88 {
            ...
            let _ = writeln!(out, "    conn: aiosqlite.Connection,");
            if !params.is_empty() {
                let _ = writeln!(out, "    *,");
                for p in params {
...
mod tests {
    ...
    #[test]
    fn test_grouped_python_aiosqlite_structs() {
        let backend = PythonAiosqliteBackend::new("sqlite").unwrap();
    ...
    #[test]
    fn test_grouped_python_aiosqlite_query_fn() {
        let backend = PythonAiosqliteBackend::new("sqlite").unwrap();
        let query = make_grouped_query();
        let result = crate::generate_with_backend(&query, &backend).unwrap();
        ...
        assert!(
            query_fn.contains("async def") && query_fn.contains("await"),
            "fn must be async; got:\n{query_fn}"
        );
    }
}
