    ...
    fn generate_query_fn(
        ...
        match &analyzed.command {
            QueryCommand::One | QueryCommand::Opt => {
                ...
                if params.is_empty() {
                    let _ = writeln!(out, "    row = conn.execute(\"\"\"{}\"\"\").fetchone()", sql);
                } else {
                    let _ = writeln!(
                        out,
            ...
            QueryCommand::Many => {
                ...
                let _ = writeln!(out, "    \"\"\"Execute {} query.\"\"\"", analyzed.name);
                if params.is_empty() {
                    let _ = writeln!(out, "    rows = conn.execute(\"\"\"{}\"\"\").fetchall()", sql);
                } else {
                    let _ = writeln!(
                        out,
            ...
            QueryCommand::ExecResult | QueryCommand::ExecRows => {
                ...
                if params.is_empty() {
                    ...
                    let _ = writeln!(out, "    result = conn.execute(\"\"\"{}\"\"\", {})", sql, args_list);
                }
                let _ = writeln!(out, "    row = result.fetchone()\n    return row[0] if row else 0");
            }
        }
