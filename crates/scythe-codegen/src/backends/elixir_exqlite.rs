...
const DEFAULT_MANIFEST_TOML: &str = include_str!("../../manifests/elixir-exqlite.toml");

pub struct ElixirExqliteBackend {
    manifest: BackendManifest,
}

impl ElixirExqliteBackend {
...
impl CodegenBackend for ElixirExqliteBackend {
    fn name(&self) -> &str {
        "elixir-exqlite"
    }
    ...
    fn generate_query_fn(
        ...
        match &analyzed.command {
            ...
            QueryCommand::ExecResult | QueryCommand::ExecRows => {
                let _ = writeln!(
                    out,
                    "@spec {}(Exqlite.Sqlite3.db(){}) :: {{:ok, non_neg_integer()}} | {{:error, term()}}",
                    func_name, param_specs
                );
            }
...
mod tests {
    ...
    use super::ElixirExqliteBackend;
    ...
    #[test]
    fn test_grouped_exqlite_structs() {
        let backend = ElixirExqliteBackend::new("sqlite").unwrap();
    ...
    #[test]
    fn test_grouped_exqlite_query_fn() {
        let backend = ElixirExqliteBackend::new("sqlite").unwrap();
        let query = make_grouped_query();
        let result = generate_with_backend(&query, &backend).unwrap();
        ...
        assert!(
            query_fn.contains("Exqlite.Sqlite3.prepare(conn, sql)"),
            "fn must use Exqlite.Sqlite3.prepare; got:\n{query_fn}"
        );
        assert!(
        ...
        assert!(
            query_fn.contains("{:ok, results}"),
            "fn must return {{:ok, results}}; got:\n{query_fn}"
        );
    }
}
