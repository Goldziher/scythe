...
///
/// board #196: a composite column is checked before the blanket `col.nullable` preamble
/// dispatch -- pgjdbc registers no type map for a user-defined composite, so
...
/// driver's text form instead and is already null-safe, so composite needs no preamble local
/// (see the matching skip in `write_exposed_nullable_preamble`).
...
fn write_exposed_nullable_preamble(
    ...
    for col in cols {
        ...
        // ~keep board #196: `exposed_rs_expr` returns the composite's `fromText` call inline
        // for every composite column, nullable or not (see its own comment) -- so no preamble
        // local is ever read for one, and emitting `rs.getObject(col, T::class.java)` here
        // would both be dead code and reintroduce the type-map-less accessor this fix removes.
        if col.neutral_type.starts_with("composite::") {
            continue;
        }
        if let Some(element) = jvm_common::array_element_neutral_type(&col.neutral_type) {
