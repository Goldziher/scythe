...
impl CodegenBackend for ElixirPostgrexBackend {
    ...
    fn generate_grouped_structs(
        ...
        // Child struct — defined first so the parent's @type can reference it without
        // forward-declaration issues in Elixir's module system.
...
mod tests {
    ...
    #[test]
    fn test_grouped_postgrex_structs() {
        ...
        assert!(
            row_struct.contains("id: integer()"),
    ...
    #[test]
    fn test_grouped_postgrex_query_fn() {
        let backend = ElixirPostgrexBackend::new("postgresql").unwrap();
        let query = make_grouped_query();
        let result = generate_with_backend(&query, &backend).unwrap();
        ...
        assert!(
            query_fn.contains("Enum.reverse(order)"),
            "fn must reverse order list to restore insertion order; got:\n{query_fn}"
        );
        assert!(
