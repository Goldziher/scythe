        ...
        match &analyzed.command {
            ...
            QueryCommand::Batch => {
                ...
                if params.len() > 1 {
                    ...
                    let _ = writeln!(
                        out,
                        "\t\t\tconn.execute({{ sqlText: `{}`, binds: [{}] as any[], complete: (err) => err ? reject(err) : resolve() }});",
                        sql,
                        args.join(", ")
                    );
                    let _ = writeln!(out, "\t\t}});");
                    ...
                    let _ = writeln!(
                        out,
                        "\t\t\tconn.execute({{ sqlText: `{}`, binds: [item] as any[], complete: (err) => err ? reject(err) : resolve() }});",
                        sql
                    );
                    let _ = writeln!(out, "\t\t}});");
                    let _ = writeln!(out, "\t}}");
...
mod tests {
    ...
    fn test_typescript_snowflake_uses_scalar_binds_for_single_execution() {
        ...
        assert!(
            query_fn.contains("binds: [id] as any[]"),
